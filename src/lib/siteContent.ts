...
function safeUrl(value: unknown, fallback: string): string {
  ...
  try {
    ...
    return parsed.protocol === 'http:' || parsed.protocol === 'https:' ? value.trim() : fallback;
  } catch {
    return fallback;
  }
}
...
export function mergeSiteContent(base: SiteContent, document: unknown): SiteContent {
  ...
  return {
    ...
    contact: {
      ...
      phoneLabel: pick(contactDoc.phoneLabel, phone !== base.contact.phone ? phone : base.contact.phoneLabel),
